init 10 python:
    ...
    class TearDisplayable(object):
        ...
        `offsetRange`: tuple[int | float, int | float]
            Минимальный / максимальный порог смещение.
        ...
        `render_child`: bool
            Если ИСТИНА, дочерние объекты будут обрабатываться под фрагментами.
        """
    ...
    class TearSurface(_BaseTear):
        ...
        `srf`: Render | Surface | GL2Model | None
            Используемая поверхность. Если `None`, делает скриншот и использует его в качестве таковой
            (размер скриншота не будет тем же, что и размер экрана, будьте осторожны).
        """
    ...
    class GlicthedDisplayable(object):
        ...
        def __call__(self, child):
            if not self.glitches: raise ValueError("Нет искажений, которые надо применить.")
