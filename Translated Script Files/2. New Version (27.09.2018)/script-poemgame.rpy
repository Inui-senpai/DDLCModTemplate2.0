...
init python:
    ...
    full_wordlist = []
    wordfile = renpy.file('poemwords.txt')
...
label poem(transition=True):
    ...
    python:
        ...
        while True:
            ...
            ui.text(pstring + '/' + str(numWords), style='poemgame_text', xpos=1030, ypos=80, color='#000') 
            for j in range(2):
                if j == 0: x = 440
                else: x = 770 
                ui.vbox()
                for i in range(5):
                    if persistent.playthrough == 3:
                        ...
                        button_text = word.word.decode('utf8')
                        wordlist.remove(word)
                    ui.textbutton(button_text, clicked=ui.returns(word), text_style='poemgame_text', xpos=x, ypos=i * 56 + ystart)
                ui.close()
