...
label ch22_main:
    ...
    "За последние пару дней я попривык и теперь чувствую себя более уверенно."
    "При входе в класс меня встречает привычная сцена."
    ...
    y 2v "Э-эм..."
    "Юри оглядывается через плечо, рассматривая комнату."
    "Нацуки сидит за партой и читает мангу."
    "А Моника, как ни удивительно, пока не пришла."
    "Юри вдруг хватает меня за руку и тащит в угол комнаты."
    show bg closet
    ...
    y 3t "А-ах..."
    y "[player]..."
    y 3u "Не говори так запросто о подобных вещах..."
    y "От этих слов меня переполняет счастье."
    y 1s "Я очень рада, что ты такой понимающий человек..."
    y "И что ты присоединился к нашему клубу."
    ...
    n 5g "Блин..."
    n 5c "Юри, ты, я так понимаю, тоже не видела?"
    show natsuki zorder 2 at t33
    show yuri zorder 3 at f32
    y 4a "..."
    "Юри явно сбита с толку тем, как спокойно к ней обращается Нацуки."
    y "Н-нет, не видела..."
    ...
    if persistent.allow_slang:
        n 2c "Юри, что за хуйню ты несёшь?"
    else:
    ...
    n "Или ты из тех, кто раздувает из мухи слона?"
    $ style.say_dialogue = style.normal
    show natsuki zorder 2 at t33
    show yuri zorder 3 at f32
    y 2o "..."
    ...
    n "Мне даже приятно это слышать, поскольку я всегда опасалась, что ты втайне ненавидишь меня или что-то в этом роде."
    n 2z "Э-хе-хе."
    show natsuki zorder 2 at t33
    show yuri zorder 3 at f32
    y 3q "В-вовсе нет!.."
    ...
    show yuri zorder 3 at f32
    y 3t "..."
    "Нацуки поворачивается ко мне."
    show yuri zorder 2 at t32
    show natsuki zorder 3 at f33
    n 2a "Между прочим, ты пока на испытательном сроке."
    show natsuki zorder 2 at t33
    mc "Эй!.."
    "Внезапно дверь с хлопком распахивается."
    ...
    m "И, если честно, там я потеряла счёт времени..."
    m "А-ха-ха..."
    show monika zorder 2 at t41
    show natsuki zorder 3 at f33
    ...
    m "Я и сама надеялась, что смогу поделиться своей песней."
    m "Думаю, поэтому я столько и репетировала в последнее время."
    mc "Ясно..."
    "Не уверен, обращается Моника ко всем в клубе или только ко мне..."
    ...
    hide monika
    "Я решаю не упоминать о том, что здесь было сказано ранее."
    "Кроме того, Нацуки уже закрылась в кладовке."
    ...
    if poemwinner[0] == 'natsuki':
        ...
        if n_appeal >= 2:
            ...
            $ style.say_dialogue = style.normal
            window hide(None)
            define audio.t6_apg = '<from ' + str(get_pos()) + ' loop 10.893>bgm/6.ogg'
            stop music
            scene black
            window auto
            $ pause(2.0)
    ...
    mc "Не торопись."
    "Юри делает глубокий вдох и достаёт из портфеля книгу."
label ch22_main2:
    if n_poemappeal[1] == 1:
        $ n_poemappeal[1] = 0
    $ poemwinner[1] = 'yuri'
...
label ch22_end:
    ...
    if faint_effect:
        define audio.t3_apg = '<from ' + str(get_pos() + 2.0) + ' loop 4.618>bgm/3.ogg'
    ...
    show monika zorder 2 at t32
    mc "Ну—"
    "Этот вопрос не из тех, на которые я могу ответить честно, ведь так?"
    show monika zorder 3 at f32
    m 1p "В общем-то..."
    m "Насколько я помню, выбора «не вступать» у тебя не было."
    show monika zorder 1 at thide
    hide monika
    "Моника садится за парту и опускает глаза."
    ...
    show natsuki zorder 1 at thide
    hide natsuki
    "Нацуки, игнорируя Юри, идёт прямиком к выходу из класса."
    ...
    $ style.say_dialogue = style.edited
    define audio.t9g_apg = '<from ' + str(get_pos() / 2.07) + ' loop 1.532>bgm/9g.ogg'
    play music t9g_apg
    y "Кому какое дело до этой несносной соплячки?"
    $ style.say_dialogue = style.normal
    define audio.t9_apg = '<from ' + str(get_pos() * 2.07) + ' loop 3.172>bgm/9.ogg'
    play music t9_apg
    hide black
    hide y_glitch_head
    ...
    "Юри повторяет тот же вопрос, что раньше задала Моника."
    "Я решаю, что уклончивый ответ лучше, чем ничего."
    mc "...Я думаю, что самое главное – чтобы все ладили друг с другом..."
    mc "...А для клуба – чтобы он мог предоставить то, что ты не сможешь получить больше нигде."
    ...
    show yuri zorder 3 at f22
    y 2v "..."
    "Юри выглядит несколько обеспокоенной, но не протестует."
    y 2t "Ладно."
    ...
    show yuri zorder 1 at thide
    hide yuri
    "Моника машет на прощание вышедшей из класса Юри."
    ...
    stop music
    m 4e "И я искренне беспокоюсь за тебя... понимаешь?"
    ...
    m 1g "Но это странно, потому что за всё время, что ты здесь, мы вместе практически не были."
    m 1n "Ах... в смысле..."
    ...
    m 1g "Стоп, ещё рано!\"{space=5000}{w=0.5}{nw}"
    m "Нет!\"{space=5000}{w=0.5}{nw}"
    m "Как это остановить?!\"{space=5000}{w=1.0}{nw}"
    window hide(None)
    window auto
    hide black onlayer front
