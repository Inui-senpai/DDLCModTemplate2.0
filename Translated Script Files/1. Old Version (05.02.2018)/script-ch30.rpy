    ...
    if process_list is not None:
        ...
        if not list(set(process_list).intersection(stream_list)):
            if currentuser != '' and currentuser.lower() != 'user' and currentuser.lower() != 'пользователь' and currentuser.lower() != player.lower():
                ...
                if currentuser.lower() == 'александр':
                    m "Кстати, [currentuser] – красивое имя."
...
label ch30_postpoem:
    ...
    m "У тебя получилось сочинить хорошее стихотворение?"
...
label ch30_endb:
    ...
    hide glitch_color onlayer front
    m "...Как ты мог?"
    m "Как ты мог поступить так со мной?"
...
label ch30_reload_2:
    ...
    if renpy.android:
        ...
        m "Ты ведь играешь в Стиме, так что тебе просто нужна зайти в свойства игры и найти кнопку «Просмотреть локальные файлы»."
...
label ch30_5:
    ...
    m "Надеюсь, ты уже не ходишь с повешенной головой из-за того случая?"
    ...
    m "Она даже повеситься нормально не смогла..."
...
label ch30_6:
    m "Кстати, мне тут некоторое время кое-что беспокоит..."
...
label ch30_9:
    m "Эх, вот здесь сейчас было пианино..."
...
label ch30_22:
    ...
    m "Представь, если бы в реальной жизни был человек похожий Юри."
...
label ch30_30:
    ...
    m "И, несмотря на такую печальную картину, это порядок никуда не девается."
...
label ch30_41:
    ...
    m "Ну и пусть! Это лишь говорит о том, что, что тебе нужно просто писать для самого себя, верно?"
    m "Да..."
    return
