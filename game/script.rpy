...
label start:
    ...
    # Эта переменная устанавливает число анти-чита для игры после прохождения Первого акта.
    ...
    #   $ mi_name = _("Майк"). 
    # Не забудьте добавить своего персонажа в «definitions.rpy»!
    $ s_name = "???"
    $ m_name = _("Девушка 3")
    $ n_name = _("Девушка 2")
    $ y_name = _("Девушка 1")
    ...
    # Эти переменные указывают, может ли игрок пропускать диалоги или переходы.
    $ allow_skipping = True
    $ config.allow_skipping = True

    ## Основная часть скрипта
    # Именно здесь и вызывается ваш сценарий!
    # «persistent.playthrough» указывает прогресс прохождения игрока, т.е. на каком он сейчас акте: 1-й, 2-й, 3-й, 4-й

    # УДАЛИТЕ ЭТУ СТРОКУ ПОСЛЕ ТОГО, КАК СОЗДАЛИ СВОЙ ФАЙЛ СЦЕНАРИЯ И ПРОПИСАЛИ ВЫЗОВ ЕГО ЛЕЙБЛА ЗДЕСЬ
    call screen dialog("Похоже, вы пытаетесь начать новую игру в мод-шаблоне без сценария.\nЭто мод-шаблон, а не самостоятельная игра. Напишите сценарий для своей модификации, пропишите его вызов в файле «script.rpy» и повторите попытку.", MainMenu(confirm=False))

    ## Пример вызова сценариев в DDLC.
    # if persistent.playthrough == 0:

    #     # Эта переменная ставит номер главы на X в зависимости от того,
    #     # какой главой игрок сейчас наслаждается.
    #     $ chapter = 0

    #     # Это выражение вызова запускает конкретный лейбл в скрипте.
    #     call ch0_main
        
    #     # Это выражение вызова запускает мини-игру про сочинение стихов.
    #     call poem
    ...
    #     # Это выражение вызова запускает мини-игру про обмен стихами.
    ...
    #     # Это выражение Python записывает файл в корневую папку игры
    #     # или в папку Android/data/имя.пакета.абв/files. <- Android/data ЗАЩИЩЕНА ОТ ЧТЕНИЯ ПОСТОРОННИМИ, ЗАМЕНИТЬ НА Android/media!
    #     python:
    #         try: open(f"{user_dir}/счхстлхвые мхсли.png", "rb")
    #         except IOError: open(f"{user_dir}/счхстлхвые мхсли.png", "wb").write(renpy.open_file("hxppy thxughts.png").read())
    ...
    #         # Это выражение вызова запускает мини-игру про сочинение стихов, но без перехода.
    #         call poem(False)
    ...
    #         # Это выражение «если» – остатки кода из наработок по DDLC,
