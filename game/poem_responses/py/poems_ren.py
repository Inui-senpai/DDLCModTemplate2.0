...
# Этот файл содержит код Python для отображения стихотворений в DDLC.

# Логика отображения стихотворений была кардинально изменена по сравнению с
# оригинальной игрой, что позволило расширить возможности управления ими.
# Также здесь используется подход Ren'Py, предполагающий использование нового файла `_ren.py` для кода Python.

# Чтобы просмотреть код отображения стихотворений, откройте файл `poems.rpy` в каталоге `poem_responses`.
...
class PoemAuthor(object):
    """
    Класс, используемый для представления автора стихотворения.
    """
    ...
    def __init__(
        ...
        """
        Инициализирует автора стихотворения с заданными параметрами.

        :param name: Имя автора.
        :param style: Применять ли конкретный стиль к стихотворению.
        :param paper: Фон стихотворения.
        :param separate_title_from_text: Нужно ли показывать название и текст одновременно.
        :param music: Музыка, которая будет играть во время показа стихотворения.
...
class Poem(renpy.text.text.Text):
    """
    Класс, используемый для представления стихотворения.
    """
    ...
    def __init__(
        ...
        **properties,
        ...
        """
        Инициализирует стихотворение с заданными параметрами.

        :param author: Автор стихотворения.
        :param text: Текст стихотворения.
        :param title: Название стихотворения.
        :param style: Применять ли конкретный стиль к стихотворению.
        :param paper: Фон стихотворения.
        :param separate_title_from_text: Нужно ли показывать название и текст одновременно.
        :param music: Музыка, которая будет играть во время показа стихотворения.
        ...
        for arg in (author, text, title):
            if not isinstance(arg, str):
                raise TypeError(f"Параметр '{arg}' должен являться строкой, а не '{type(arg).__name__}'.")
    ...
    def format_music_str(self, music: str, pos: int = 0):
        """
        Возвращает форматированную строку для музыки, которая будет играть во время показа стиха и после его скрытия.

        :param music: Аудиозапись, которую нужно форматировать.
        :param pos: Позиция для начала воспроизведения.

        :type music: str
        :type pos: int

        :return music: Форматированная строка для музыки.
        :rtype: str
        """
    ...
    def show(
        ...
        """
        Показывает стихотворение во время обмена стихами.

        :param paper_sound: Звук, которым будет сопровождаться появление стиха.
        :param music: Должен ли стих иметь музыкальное сопровождение.
        :param from_current: Должна ли музыка играть с текущей позиции предыдущей музыки.
        :param revert_music: Возвращать ли предыдущую музыку после скрытия стихотворения.
        :param testing: Не используется в DDLC. Используется системой GitHub Actions для тестирования.
...
class PoemResponseDB(object):
    """
    Класс, представляющий базу данных стихотворений.
    """

    def __init__(self):
        """
        Инициализирует базу данных для обмена стихами.
        """
    ...
    def add_poem(
        ...
        """
        Добавляет стихотворение в базу данных.

        :param identifier: Уникальный идентификатор стихотворения.
        :param author: Автор стихотворения.
        :param title: Название стихотворения.
        :param text: Текст стихотворения.
        :param style: Применять ли конкретный стиль к стихотворению.
        ...
        :param separate_title_from_text: Нужно ли показывать название и текст одновременно.
        :param music: Музыка, которая будет играть во время показа стихотворения.
        :param translate: Позволять ли Ren'Py переводить стихотворение.
    ...
    def get_poem(self, identifier: str) -> Poem:
        """
        Выдаёт стихотворение из базы данных по его идентификатору.

        :param identifier: Уникальный идентификатор стихотворения.

        :type identifier: str

        :return poem: Стихотворение, если оно есть в базе.
        :rtype: Poem

        :raise ValueError: Если стихотворения с данным идентификатором не существует.
        """
        if identifier in self.poems:
            return self.poems[identifier]
        raise ValueError(f"Не удалось найти стихотворение с идентификатором '{identifier}'.")

    def get_poems(self) -> list[str]:
        """
        Возвращает список всех стихотворений в базе данных.

        :return: Список всех стихотворений.
        :rtype: list[Poem]
        """
    ...
    def show_poem(self, identifier: str, img: str | None = None, **kwargs):
        """
        Выводит конкретное стихотворение из базы данных на экран.

        :param identifier: Уникальный идентификатор стихотворения.
        :param kwargs: Дополнительные ключевые аргументы, которые нужно передать методу `show` класса Poem.
...
# Инициализирует базу данных обмена и его участников.
...
## Стихотворения Юри во втором акте
poem_db.add_poem(
    "poem_y22",
    author_y,
    title="Колесо",
...
находящего выхода напряжения всего моего тела. Её третий глаз притягивает меня ближе.""",
    ...
    style="yuri_text_3",
    # translate="none", <- Азя явно не до конца читал «стих»...
)

## Стихотворения Нацуки
poem_db.add_poem(
    "poem_n1",
    author_n,
...
## Стихотворения Нацуки во втором акте
poem_db.add_poem(
    "poem_n2b",
    author_n,
...
NC90L7QstC40YLRjNGB0Y8u""",
    # translate="none", <- это тоже вполне переводимо (шифр Base64)
)
...
## Стихотворения Моники во втором акте
poem_db.add_poem(
    "poem_m21",
    author_m,
    title="Дыра в стене",
